...
class singleBay extends Component {
  ...
  componentDidMount() {
    ...
    var receivedItems1 = items.filter(
      (item) => item.received === true && item.starred === true
    );
    var receivedItems2 = items.filter(
      (item) => item.received === true && item.starred === false
    );
    var notReceivedItems1 = items.filter(
      (item) => item.received === false && item.starred === true
    );
    var notReceivedItems2 = items.filter(
      (item) => item.received === false && item.starred === false
    );
    // var arrangedItems = notReceivedItems.concat(receivedItems);
    var arrangedItems = notReceivedItems1
      .concat(notReceivedItems2)
      .concat(receivedItems1)
      .concat(receivedItems2);
    this.setState({
    ...
    return (
      ...
      >
        <div className="singleBay_header">
          <a href={bayNum} target="_blank">
            {bayNum}
          </a>{" "}
          <a href={"/edit/" + bayNum}>(edit)</a>
